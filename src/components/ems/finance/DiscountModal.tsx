...
export default function DiscountModal({ studentId: propStudentId, feeStructureId, invoiceId, invoiceAmount, onClose, onSave }: Props) {
  const [students, setStudents] = useState<Student[]>([])
  const [feeStructures, setFeeStructures] = useState<FeeStructureOption[]>([])
  const [selectedStudent, setSelectedStudent] = useState(propStudentId?.toString() || "")
  const [selectedFeeStructure, setSelectedFeeStructure] = useState(feeStructureId?.toString() || "")
  ...
  useEffect(() => {
    api.get('/ems/students?limit=500')
      .then(res => {
        if (res.data.success) setStudents(res.data.data?.data || res.data.data || [])
      })
      .catch(() => {})
      ...
      .then(res => {
        ...
        for (const item of data) {
          ...
          if (fs && !seen.has(item.fee_structure_id)) {
            ...
            unique.push({ id: item.fee_structure_id, fee_type: fs.fee_type, amount: fs.amount, course_id: fs.course_id })
          }
        }
    ...
    try {
      ...
      if (discountRes.data.success) {
        if (invoiceId && invoiceAmount) {
          const discountAmount = amount > 0 ? amount : (invoiceAmount * percentage / 100)
        ...
        toast.success(`Discount applied! ₹${discountRes.data.data?.id ? 'Done' : ''}`)
        onSave()
        onClose()
      }
    } catch (err: any) {
              ...
              >
                ...
                {feeStructures.map(fs => (
                  <option key={fs.id} value={fs.id}>
                    #{fs.id} — {fs.fee_type} (₹{Number(fs.amount).toLocaleString()})
                  </option>
                ))}
              </select>
            </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={e => {
                ...
                const selected = DISCOUNT_TYPES.find(d => d.id === e.target.value)
