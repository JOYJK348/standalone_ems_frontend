...
export const AttendanceVerification = ({
    ...
    const pendingSessions = availableSessions.filter(
        (s) => s.recommended_action !== "COMPLETED",
    );
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (err) => {
                ...
                navigator.geolocation.getCurrentPosition(
                    ...
                    () =>
                        toast.error(
                            "Could not get GPS. Please ensure Location is enabled.",
                        ),
    ...
    const handleConfirmAttendance = async () => {
        ...
        try {
            ...
            const response = await api.post(
                "/ems/attendance?mode=student-mark",
                {
    ...
    if (step === "NEED_REGISTRATION") {
        return (
            <FaceRegistration
                onSuccess={() => setStep("SELECT_SESSION")}
                onClose={onClose}
            />
        );
    }
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
            <div className="w-full max-w-md bg-white rounded-[2.5rem] overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col">
                <button
                    onClick={() => {
                        stopStream();
                        onClose();
                    }}
                    className="absolute top-6 right-6 z-10 p-2 bg-gray-100/80 hover:bg-gray-200 rounded-full transition-colors font-black"
                >
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-blue-100 text-[10px] font-black uppercase tracking-widest opacity-80">
                                {step === "CONFIRM" ||
                                (selectedSession &&
                                    selectedSession.require_face_verification ===
                                        false)
                                    ? "Action Confirmation"
                ...
                <div className="p-8 overflow-y-auto flex-1">
                    {step === "SELECT_SESSION" && (
                        <div className="space-y-6">
                                ...
                                {availableSessions.map((session, idx) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`group relative p-6 rounded-[2.5rem] border-2 transition-all shadow-lg ${
                                                isCompleted
                                        ...
                                        >
                                            <div className="flex items-center justify-between pointer-events-none">
                                                <div>
                                                    <h4
                                                        className={`font-black text-lg ${isCompleted ? "text-gray-400" : isExit ? "text-orange-900 group-hover:text-white" : "text-green-900 group-hover:text-white"}`}
                                                    >
                                                        {
                                                            session.course
                                                                ?.course_name
                                                    ...
                                                    <p
                                                        className={`text-xs font-bold uppercase tracking-wider ${isCompleted ? "text-gray-300" : isExit ? "text-orange-600 group-hover:text-orange-100" : "text-green-600 group-hover:text-green-100"}`}
                                                    >
                                                        {session.session_type}
                                                    </p>
                                                </div>
                                                <div
                                                    className={`p-3 rounded-full ${isCompleted ? "bg-gray-100" : isExit ? "bg-orange-100 group-hover:bg-white/20" : "bg-green-100 group-hover:bg-white/20"}`}
                                                >
                                                    {isCompleted ? (
                                                        <CheckCircle2 className="h-6 w-6 text-gray-300" />
                                                    ) : isExit ? (
                                                        <User className="h-6 w-6 text-orange-600 group-hover:text-white" />
                                                    ) : (
                                            ...
                                            <div className="mt-4 flex items-center justify-between">
                                                ...
                                                <div
                                                    className={`px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-sm border ${
                                                        isCompleted
                                ...
                                {availableSessions.length === 0 && (
                                    <div className="py-12 text-center bg-gray-50/50 rounded-[2.5rem] border-2 border-dashed border-gray-100 flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-gray-500 text-xs font-bold uppercase tracking-widest mt-1">
                                            Currently no active attendance
                                            sessions found
                                        </p>
                                        <Button
                                            variant="ghost"
                            ...
                            {availableSessions.length > 0 &&
                                pendingSessions.length > 0 && (
                                    <p className="text-center text-[10px] text-gray-400 font-black uppercase tracking-widest pt-4 italic">
                                        Select your active session to proceed
                                        with biometric scan
                                    </p>
                                )}
                            {availableSessions.length > 0 &&
                                pendingSessions.length === 0 && (
                                    <p className="text-center text-[10px] text-gray-500 font-black uppercase tracking-widest pt-4">
                                        All today's sessions are marked for now.
                                        Check back during exit time.
                                    </p>
                                )}
                        </div>
                    )}
                        ...
                        <div className="space-y-6">
                            <div className="relative aspect-square rounded-[2.5rem] overflow-hidden bg-gray-950 border-4 border-gray-50 shadow-2xl">
                                ...
                                <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
                                    ...
                                    >
                                        <RefreshCw
                                            className={`h-3.5 w-3.5 ${isCapturing ? "animate-spin" : "group-hover:rotate-180 transition-transform duration-700"}`}
                                        />
                                ...
                                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                    <div
                                        className={`w-64 h-64 border-2 rounded-full transition-all duration-500 ${faceDetected ? "border-green-400 scale-105" : "border-white/20 border-dashed"}`}
                                    />
                                </div>
                                <div className="absolute top-4 left-4">
                                    <div
                                        className={`px-4 py-2 rounded-full text-[10px] font-black tracking-widest uppercase backdrop-blur-md border ${faceDetected ? "bg-green-500 text-white border-green-400" : "bg-black/40 text-white border-white/20"}`}
                                    >
                                        {selectedSession?.course?.course_name.substring(
                                            0,
                                            15,
                                        )}
                                        ...
                                ...
                                <div className="absolute bottom-4 left-1/2 -translate-x-1/2">
                                    <div
                                        className={`flex items-center gap-2 px-3 py-1.5 bg-black/60 backdrop-blur-md rounded-xl text-white text-[10px] font-bold border ${location ? "border-green-400/30" : "border-white/10"}`}
                                    >
                                        <MapPin
                                            className={`h-3 w-3 ${location ? "text-green-400" : "text-gray-400 animate-pulse"}`}
                                        />
                                        {location
                                            ? "GPS VALID"
                                            : "GPS SEARCHING..."}
                            ...
                            <Button
                                ...
                                disabled={!faceDetected && step === "SCANNING"}
                                className="w-full h-14 bg-blue-600 hover:bg-blue-700 rounded-2xl font-bold shadow-xl flex items-center justify-center gap-3 text-white text-sm uppercase tracking-wider"
                            >
                                <ShieldCheck
                                    className={`h-5 w-5 ${faceDetected ? "animate-bounce" : ""}`}
                                />
                    ...
                    {step === "CONFIRM" && (
                        <div className="flex flex-col h-full">
                            <div className="text-center py-8 px-4 space-y-4 flex-1 flex flex-col items-center justify-center">
                                ...
                                <div
                                    className={`mt-6 p-4 rounded-2xl w-full max-w-xs border ${
                                        location
                    ...
                    {step === "ERROR" && (
                        <div className="text-center py-8 space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-red-500 font-medium text-sm bg-red-50 p-4 rounded-2xl border border-red-100">
                                    {error}
                                </p>
                            </div>
