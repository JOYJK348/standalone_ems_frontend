...
export default function FeesPage() {
    ...
    const [showEditModal, setShowEditModal] = useState<FeeStructure | null>(
        null,
    );
    ...
    const fetchCourses = async () => {
        try {
            ...
            if (res.data.success)
                setCourses(res.data.data?.courses || res.data.data?.data || []);
        } catch {}
    };
    ...
    const toggleActive = async (fee: FeeStructure) => {
        try {
            const res = await api.put(`/ems/fees?id=${fee.id}`, {
                is_active: !fee.is_active,
            });
            if (res.data.success) {
                toast.success(
                    `Fee ${fee.is_active ? "deactivated" : "activated"}`,
                );
                fetchFees();
            }
        } catch {
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">
                                Manage fee structures for courses
                            </p>
                        </div>
                        <div className="flex gap-2">
                            <Link href="/ems/dynamic-role/finance-manager/fee-structure">
                                ...
                                >
                                    <Layers className="h-4 w-4 mr-2" /> Create
                                    with Installments
                                </Button>
                            </Link>
                            <Button
                            ...
                            {loading ? (
                                ...
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-sm text-gray-400 mt-1">
                                        Add fee structures for your courses
                                    </p>
                                </div>
                            ) : (
                                                ...
                                                >
                                                    ...
                                                    <td className="py-3 pr-4">
                                                        <span className="text-sm text-gray-600">
                                                            {fee.courses
                                                                ? `${fee.courses.course_name} (${fee.courses.course_code})`
                                                    ...
                                                    <td className="py-3 pr-4">
                                                        <span
                                                            className={`text-xs font-bold ${fee.is_mandatory ? "text-blue-600" : "text-gray-400"}`}
                                                        >
                                                            {fee.is_mandatory
                                                                ? "Yes"
                                                                : "No"}
