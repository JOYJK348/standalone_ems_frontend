...
import {
  Search, CreditCard, Plus, Loader2, ArrowLeft, ArrowRight,
...
interface Payment {
  ...
  remarks: string | null
  students: { first_name: string; last_name: string; email: string; phone: string } | null
}
...
export default function PaymentsPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setForm({ student_id: "", amount_paid: "", payment_method: "CASH", transaction_id: "", remarks: "" })
        fetchPayments()
      }
    } catch (err: any) {
  ...
  const statusBadge = (status: string) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold ${isCompleted ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Record and track student payments</p>
            </div>
            ...
            <CardContent className="p-4">
              ...
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 mt-1">Record your first payment to get started</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead className="border-b border-gray-100">
                      <tr>
                        <th className="pb-3 text-xs font-bold text-gray-400 uppercase">Receipt #</th>
                        ...
                        <th className="pb-3 text-xs font-bold text-gray-400 uppercase">Method</th>
                      ...
                      {payments.map(pmt => (
                        <tr key={pmt.id} className="hover:bg-gray-50 transition-colors">
                          <td className="py-3 pr-4">
                            <span className="font-bold text-gray-900">{pmt.receipt_number}</span>
                          </td>
                          <td className="py-3 pr-4">
                            <span className="text-sm font-medium text-gray-900">
                              {pmt.students ? `${pmt.students.first_name} ${pmt.students.last_name}` : `Student #${pmt.student_id}`}
                            </span>
                          </td>
                          <td className="py-3 pr-4">
                            <span className="font-bold text-gray-900">₹{Number(pmt.amount_paid).toLocaleString()}</span>
                          </td>
                          <td className="py-3 pr-4">
                            <span className="text-sm text-gray-600">{pmt.payment_method}</span>
                          </td>
                          <td className="py-3 pr-4">
                            <span className="text-sm text-gray-600">{new Date(pmt.payment_date).toLocaleDateString()}</span>
                          </td>
                          <td className="py-3">{statusBadge(pmt.payment_status)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
