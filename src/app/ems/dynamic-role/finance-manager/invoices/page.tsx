...
export default function InvoicesPage() {
    ...
    const [showStatusModal, setShowStatusModal] = useState<Invoice | null>(
        null,
    );
    const [showDiscountModal, setShowDiscountModal] = useState<Invoice | null>(
        null,
    );
    ...
    const handleStatusUpdate = async (id: number, status: string) => {
        try {
            setSubmitting(true);
            const res = await api.put(`/ems/invoices?id=${id}&mode=status`, {
                status,
            });
            if (res.data.success) {
    ...
    const statusBadge = (status: string) => {
        ...
        return (
            <span
                className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold ${m.bg} ${m.text}`}
            >
                            ...
                            <div key={i}>
                                <Card className="border-0 shadow-sm">
                                    <CardContent className="p-4 flex items-center gap-3">
                                        <div
                                            className={`w-10 h-10 rounded-lg ${colorMap[s.color].bg} flex items-center justify-center`}
                                        >
                                            <s.icon
                                                className={`h-5 w-5 ${colorMap[s.color].text}`}
                                            />
                                        </div>
                                        <div>
                                ...
                                {useGst && gstBreakup && (
                                    <div className="bg-gray-50 p-4 rounded-lg space-y-2 text-sm">
                                        <div className="flex justify-between text-gray-500">
                                            <span>
                                                GSTIN: {gstBreakup.gstin}
                                            </span>
                                            <span>
                                                HSN: {gstBreakup.hsn_code}
                                            </span>
                                        </div>
                                        <div className="border-t pt-2 space-y-1">
                                            <div className="flex justify-between">
                                                ...
                                                <span className="font-medium">
                                                    ₹
                                                    {gstBreakup.taxable_amount.toLocaleString()}
                                                </span>
                                            </div>
                                            <div className="flex justify-between text-gray-600">
                                                <span>
                                                    CGST ({gstBreakup.cgst_rate}
                                                    %)
                                                </span>
                                                <span>
                                                    ₹
                                                    {gstBreakup.cgst_amount.toLocaleString()}
                                                </span>
                                            </div>
                                            <div className="flex justify-between text-gray-600">
                                                <span>
                                                    SGST ({gstBreakup.sgst_rate}
                                                    %)
                                                </span>
                                                <span>
                                                    ₹
                                                    {gstBreakup.sgst_amount.toLocaleString()}
                                                </span>
                                            </div>
                                            <div className="flex justify-between font-bold border-t pt-2 text-base">
                                                ...
                                                <span>
                                                    ₹
                                                    {gstBreakup.total_amount.toLocaleString()}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        >
                            ...
                            <p className="text-sm text-gray-500 mb-4">
                                ...
                                <span className="font-bold text-gray-700">
                                    {showStatusModal.invoice_number}
                                </span>
                            </p>
