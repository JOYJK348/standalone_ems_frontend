...
export default function CreateRolePage() {
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    try {
      const res = await api.post('/ems/roles', {
        role_name: roleName.trim(),
        description: description.trim() || null,
        menu_ids: selectedMenus
      })
      const data = res.data
      if (!data.success) { setError(data.error?.message || 'Failed to create'); return }
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-4xl space-y-6">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-900">Create Role</h1>
          <p className="text-sm text-slate-500">Define a new custom role with menu-level access</p>
        </div>
