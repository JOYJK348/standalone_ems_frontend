...
export default function StudentsPage() {
    ...
    const handleEnrollStudent = async (e: React.FormEvent) => {
        ...
        try {
            const response = await api.post("/ems/enrollments", {
                ...enrollFormData,
                student_id: enrollFormData.student_id
                    ? parseInt(enrollFormData.student_id)
                    : null,
                course_id: enrollFormData.course_id
                    ? parseInt(enrollFormData.course_id)
                    : null,
                batch_id: enrollFormData.batch_id
                    ? parseInt(enrollFormData.batch_id)
                    : null,
            });
    ...
    const handleAllocatePractice = async (
        studentId: number,
        moduleType: string,
    ) => {
        try {
            ...
            toast.error(
                error.response?.data?.message || "Failed to allocate lab",
            );
        } finally {
            setAllocating(false);
        }
    ...
    const handleResetPractice = async (allocationId: number) => {
        try {
            setAllocating(true);
            const response = await api.post("/ems/practice/manager/reset", {
                allocationId,
            });
            if (response.data.success) {
    ...
    const fetchStudentAllocations = async (studentId: number) => {
        try {
            ...
            const response = await api.get(
                `/ems/practice/student/status?student_id=${studentId}`,
            );
            if (response.data.success) {
                ...
                {showEnrollForm && (
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <div>
                                    <Label htmlFor="enroll_course_id">
                                        Select Course *
                                    </Label>
                                    <select
                                        id="enroll_course_id"
                                ...
                                <div className="space-y-6 text-left">
                                        ...
                                        <div className="flex items-center justify-between group">
                                            <div>
                                                ...
                                                <p className="font-mono font-bold text-gray-800 text-sm">
                                                    {
                                                        generatedCredentials.student_code
                                                    }
                                                </p>
                                            </div>
                                            <Button
                                    ...
                                    <div className="flex flex-col gap-3">
                                        <div className="flex items-start gap-3 p-4 bg-blue-50/50 rounded-2xl text-blue-700 text-[11px] leading-relaxed">
                                            ...
                                            <p>
                                                Account secured and ready. share
                                                the "Welcome Kit" below with the
                                                student for their first login.
                                            </p>
                                        </div>
                                        <Button
                                            type="button"
                                            ...
                                            onClick={() => {
                                                ...
                                                toast.success(
                                                    "Welcome Kit copied to clipboard!",
                                                );
                                            }}
                                        >
                                            Copy Welcome Kit
                        ...
                        >
                            <div className="bg-gradient-to-br from-orange-500 to-red-600 px-6 py-6 text-center text-white relative">
                                ...
                                <h2 className="text-xl font-bold">
                                    Practical Lab Allocation
                                </h2>
                                <p className="text-orange-50/80 text-xs">
                                    Assign simulation modules to student
                                </p>
                            </div>

                            <div className="p-6 space-y-4">
                                {["GST", "TDS", "INCOME_TAX"].map((type) => {
                                    const allocation = studentAllocations.find(
                                        (a) => a.module_type === type,
                                    );
