...
export default function MaterialsPage() {
    ...
    const [viewMode, setViewMode] = useState<"LIST" | "FORM" | "PREVIEW">(
        "LIST",
    );
    ...
    const handleUploadMaterial = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.data.success) {
                toast.success(
                    editingMaterialId ? "Module updated" : "Module created",
                );
                setViewMode("LIST");
                fetchMaterials();
                // Reset form
    ...
    const filteredMaterials = (materials || []).filter((material) => {
        ...
        const matchesTab =
            activeTab === "ALL" ||
            (activeTab === "COURSE" && material.course_id);
    ...
    const getFileIcon = (fileType: string) => {
        ...
        if (type === "VIDEO")
            return <VideoIcon className="h-8 w-8 text-indigo-500" />;
        if (type === "IMAGE")
            return <ImageIcon className="h-8 w-8 text-sky-500" />;
        if (type === "LINK")
            return <Link2 className="h-8 w-8 text-emerald-500" />;
            ...
            <AcademicManagerLayout>
                ...
                <main className="flex-1 p-4 sm:p-8 max-w-[1600px] mx-auto w-full">
                            ...
                            {loading ? (
                                ...
                                <Card className="border-0 shadow-lg p-12 text-center">
                                    ...
                                    <p className="text-gray-600 mb-6">
                                        Your search didn't match any materials.
                                        Try a different query or add a new
                                        resource.
                                    </p>
                                </Card>
                            ) : (
                                        ...
                                        <div className="space-y-6">
                                            {contentSections.map(
                                                (section, idx) => (
                                                    <div
                                                        key={idx}
                                                        className="p-6 bg-slate-50/50 rounded-2xl border border-slate-200 relative group transition-all hover:bg-slate-50 hover:shadow-md"
                                                    >
                                                        <div className="absolute top-4 right-4 flex items-center gap-2">
                                                            <span className="px-2 py-1 bg-slate-200 rounded-lg text-[10px] font-bold text-slate-500">
                                                                {idx + 1}
                                                            </span>
                    ...
                    {viewMode === "PREVIEW" && selectedMaterial && (
                        <div className="flex flex-col h-[85vh] bg-white rounded-[2.5rem] border border-slate-200/60 overflow-hidden shadow-2xl shadow-slate-200/50">
                            ...
                            <div className="flex-1 flex overflow-hidden bg-slate-50/50 relative justify-center items-center p-4">
                                {(() => {
                                    ...
                                    if (
                                        ...
                                        selectedMaterial.content_json.length > 0
                                    ) {
                                        const section =
                                            selectedMaterial.content_json[
                                                previewSectionIdx
                                            ] ||
                                            selectedMaterial.content_json[0];

                                        const ytMatch =
                                            section.video_url?.match(
                                                /^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/,
                                            );
                                        const ytEmbed =
                                            ytMatch && ytMatch[2].length === 11
                                                ? `https://www.youtube.com/embed/${ytMatch[2]}`
                                                : null;
                                                    ...
                                                    <div className="w-64 bg-white border-r border-slate-200 overflow-y-auto hidden md:block flex-shrink-0">
                                                        <div className="p-4">
                                                            ...
                                                            <div className="space-y-1">
                                                                {selectedMaterial.content_json.map(
                                                                    (
                                                                        s: any,
                                                                        i: number,
                                                                    ) => (
                                                                        ...
                                                                        >
                                                                            <div
                                                                                className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 ${previewSectionIdx === i ? "bg-white text-indigo-700" : "bg-slate-100 text-slate-400"}`}
                                                                            >
                                                                                {i +
                                                                                    1}
                                                                            </div>
                                    ...
                                    const type =
                                        selectedMaterial.material_type?.toUpperCase();
                                    if (
                                        type === "VIDEO" ||
                                        ...
                                        return (
                                            <iframe
                                                src={embedUrl}
                                                className="w-full h-full max-w-5xl aspect-video rounded-2xl shadow-xl"
                                                allowFullScreen
                                            />
                                        );
                                    }
