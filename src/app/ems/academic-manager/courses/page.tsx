...
export default function CoursesPage() {
    ...
    const handleEditCourse = (course: Course) => {
        setFormData({
            ...
            enabled_practice_modules: course.enabled_practice_modules ? course.enabled_practice_modules : [],
                                ...
                                <Card className="border-0 shadow-lg hover:shadow-xl transition-all group overflow-hidden">
                                    <div className="relative h-40 w-full overflow-hidden bg-purple-50">
                                        ...
                                        <div className="absolute top-4 right-4">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest backdrop-blur-md border ${course.status === 'PUBLISHED' || course.status === 'ACTIVE'
                        ...
                        >
                            ...
                            <form onSubmit={handleCreateCourse} className="p-6 space-y-6">
                                ...
                                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-200">
                                    <div>
                                        <p className="text-sm font-semibold text-gray-700">Course Status</p>
                                        <p className="text-xs text-gray-500">{formData.is_published ? 'Published & Active' : 'Draft — not visible to students'}</p>
                                    </div>
                                    <button
                                        type="button"
                                        onClick={() => setFormData({
                                            ...
                                            status: formData.is_published ? 'DRAFT' : 'PUBLISHED'
                                        })}
                                    ...
                                    >
                                        <span className={`absolute top-0.5 left-0.5 w-6 h-6 bg-white rounded-full shadow transition-transform ${
                                            formData.is_published ? 'translate-x-7' : ''
                                        }`} />
                                    </button>
                                </div>
