...
export default function AttendancePage() {
    ...
    const fetchAttendance = async () => {
        try {
            ...
            const response = await api.get(
                "/ems/attendance?mode=student-history",
            );
    ...
    const fetchActiveSessions = async () => {
        try {
            ...
            if (
                ...
                response.data.data.active_attendance_sessions
            ) {
                ...
                const pending = sessions.filter(
                    (s: any) => s.recommended_action !== "COMPLETED",
                );
    ...
    const handleAttendanceClick = () => {
        console.log("Handle attendance click triggered", {
        ...
        if (activeSessions && activeSessions.length > 0) {
            ...
            toast.info(
                "No active attendance sessions available at the moment.",
            );
        }
    };
                        ...
                        <CardContent className="p-6 sm:p-8 relative">
                            ...
                            <div className="mt-6 flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <span
                                        className={`px-4 py-2 rounded-full text-sm font-bold ${statusInfo.bg} ${statusInfo.color} border-2 border-white/20`}
                                    >
                ...
                <div className="mb-6">
                    <div className="w-full">
                        <Card
                            ...
                            className={`border-0 shadow-lg overflow-hidden transition-all group ${
                                pendingCount > 0
                            ...
                            <CardContent className="p-5">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="hidden sm:block">
                                        <div
                                            className={`p-2 rounded-full transition-colors ${pendingCount > 0 ? "bg-white/20 group-hover:bg-white/30" : "bg-gray-200"}`}
                                        >
                                            <ArrowLeft
                                                className={`h-6 w-6 rotate-180 ${pendingCount > 0 ? "text-white" : "text-gray-400"}`}
                                            />
                                        </div>
                                    </div>
                                    <div className="sm:hidden">
                                        <div
                                            className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${pendingCount > 0 ? "bg-white/20 group-hover:bg-white/30" : "bg-gray-200"}`}
                                        >
                                            <ArrowLeft
                                                className={`h-5 w-5 rotate-180 ${pendingCount > 0 ? "text-white" : "text-gray-400"}`}
                                            />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-orange-800">
                                            ...
                                            Maintain at least 75% attendance to
                                            be eligible for exams.
                                        </p>
                                    </div>
                                </div>
                            </CardContent>
                            ...
                            <Card className="border-0 shadow-md bg-gradient-to-br from-purple-600 to-indigo-600">
                                <CardContent className="p-5 text-white">
                                    ...
                                    <p className="text-sm text-white/90 mb-4">
                                        Attendance is verified using advanced
                                        Face Recognition and GPS technology.
                                    </p>
                    ...
                    onSuccess={(session: any) => {
                        ...
                        if (
                            ...
                            (session.class_mode === "ONLINE" ||
                                session.class_mode === "HYBRID")
                        ) {
                            if (session.live_class?.meeting_link) {
                                window.open(
                                    session.live_class.meeting_link,
                                    "_blank",
                                );
